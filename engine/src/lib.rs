...
const MASSIVE_NUMBER: i32 = 1000000000;

pub fn alpha_beta_evaluate(board: &mut BoardState, depth: u32, mut alpha: i32, beta: i32) -> i32 { // Negamax
    if depth == 0 {
    ...
    for move_ in move_list {
        ...
        let value = -alpha_beta_evaluate(board, depth - 1, -beta, -alpha);
...
pub fn get_best_moves(board: &mut BoardState, depth: u32) -> Vec<(Move, i32)> {
    ...
    for move_ in move_list.iter_mut() {
        ...
        eval_list.push((*move_, alpha_beta_evaluate(board, depth - 1, -MASSIVE_NUMBER, MASSIVE_NUMBER)));
        move_.unmake_move(&mut board.board);
    }

    eval_list.sort_by(|a, b| {
        if a.1 < b.1 {
...
mod tests {
    ...
    fn test_quick_eval() {
        ...
        println!("{}", alpha_beta_evaluate(&mut board, 3, -1000000, 10000000));

        let best_move = get_best_move(&mut board, 3);
        println!("Best move: {}{}", square_to_algebraic(best_move.from), square_to_algebraic(best_move.to));
        println!("\nBest move list:");
        for eval in get_best_moves(&mut board, 3) {
