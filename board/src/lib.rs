...
pub fn print_move(move_: moves::Move) {
    if move_.promotion_type != 'z' {
        ...
        println!("{}{}", square_to_algebraic(move_.from), square_to_algebraic(move_.to));
    }
}

fn print_divide(move_: moves::Move, value: u64) {
    if move_.promotion_type != 'z' {
        println!("{}{}{}: {}", square_to_algebraic(move_.from), square_to_algebraic(move_.to), move_.promotion_type, value);
    } else {
        println!("{}{}: {}", square_to_algebraic(move_.from), square_to_algebraic(move_.to), value);
    }
}

pub fn count_moves(board: &mut board::Board, castle_rights: u32, white: bool, mut en_passant: u64, depth: u32) -> u64 {
    let mut perft = 0;
...
fn recursive_count_moves(board: &mut board::Board, move_: moves::Move, white: bool, depth: u32) -> u64 {
    let mut perft = 0;
...
pub fn count_moves_no_threads(board: &mut board::Board, castle_rights: u32, white: bool, mut en_passant: u64, depth: u32) -> u64 {
    let mut perft = 0;
